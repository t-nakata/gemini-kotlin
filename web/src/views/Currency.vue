                ...
                <el-table
                    ...
                    <el-table-column
                        ...
                        <template slot-scope="scope">
                            <el-button size="mini" type="danger" @click="deleteCurrency(scope.row.id)">x</el-button>
                        </template>
                    </el-table-column>
                </el-table>
            </el-card>
  ...
  export default {
    ...
    methods: {
        refresh: async function () {
            const res = await axios.get('http://localhost:8080/api/currency/')
            this.currencies = res.data.currencies
            this.request.name = undefined
            this.request.symbol = undefined
        },
        addCurrency: async function () {
            await axios.post('http://localhost:8080/api/currency/', this.request)
            await this.refresh()
            this.$message({
                showClose: true,
        ...
        deleteCurrency: async function (id) {
            await axios.delete('http://localhost:8080/api/currency/'+ id)
            await this.refresh()
            this.$message({
                showClose: true,
